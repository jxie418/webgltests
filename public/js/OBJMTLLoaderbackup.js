...
THREE.OBJMTLLoader.prototype = {
	...
	load: function (car,damageView, onLoad , onProgress, onError) {
		...
		mtlLoader.load(damageView.carFolder+"mtl.mtl", function ( materials ) {
			...
			var totalNumber = (damageView.objFileNames.length + 4) * 100;
			var onProgress = function ( xhr ) {
				if ( xhr.lengthComputable ) {
					...
					var totalPercentComplete  = damageView.loadedCount / totalNumber * 100;
					console.log( Math.round(totalPercentComplete, 2) + '% downloaded' );
				}
			};
						...
						if ( materialName ) {
							...
							if(damageView.loadedMaterials.hasOwnProperty(materialName) !== true) {
								...
								if(materialName.indexOf("paint_") > -1) {
									...
									material = new BABACAR.LightsGlassMaterial( damageView.reflections );
								} else if(materialName.indexOf("windowglass_") > -1)  {
									material = new BABACAR.WindowsGlassMaterial( damageView.reflections );
								} else if (materialName.indexOf("shadow")>-1) {
									material = new BABACAR.ShadowsMaterial(damageView.carFolder);
								}
